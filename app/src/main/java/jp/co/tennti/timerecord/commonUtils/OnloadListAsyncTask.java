...
public class OnloadListAsyncTask extends AsyncTask<String, Integer, List<HashMap<String, String>>> implements DialogInterface.OnCancelListener {
    ...
    protected List<HashMap<String, String>> doInBackground(String... params) {
        ...
        /**DB接続 実行処理**/
        try {
            cursor = db.rawQuery("SELECT basic_date,leaving_date,over_time_date,week FROM "
                    + timeUtil.createTableName() +
                    " WHERE year_month_date=? ORDER BY basic_date LIMIT 31;", new String[]{timeUtil.getCurrentYearMonthHyphen()});
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                map.put("over_time_date", cursor.getString(cursor.getColumnIndex("over_time_date")));
